...
function EditVegetable() {
  ...
  // Pobierz warzywo i dostępne kategorie
  useEffect(() => {
    ...
    axios.get('http://localhost:5000/api/vegetables')
      .then(res => {
        const vegetable = res.data.find(v => v.id === parseInt(id));
        if (vegetable) {
          setFormData({
            name: vegetable.name || '',
  ...
  const handleSubmit = e => {
    e.preventDefault();
    axios.put(`http://localhost:5000/api/vegetables/${id}`, formData)
